...
public class Cuenta
{
	...
	// Métodos de clase
	// Constructores
	Cuenta(String e, String s, String d, String n, String i)
	{
		Entidad = e;
		Sucursal = s;
		Dc = d;
	...
	boolean CuentaCorrecta()
	{
		...
		String cuenta = this.Entidad+this.Sucursal+this.Dc+this.NCuenta;
	...
	String CalcularIban()
	{
		// Se suma al némero de cuenta el código del país según la tabla
		...
		//Z=35
		// Para España seré el ES00 y sustituyendo E por 14 y S por 28 tendríamos el iban original
		// Se concatena por detrás al número de cuenta
		String iban = "";
		String
		iban0=this.Entidad+this.Sucursal+this.Dc+this.NCuenta+"142800";
		BigInteger iban1 = new BigInteger(iban0);
		// Calculamos su módulo 97
		int iban2 = Integer.parseInt((iban1.mod(new BigInteger("97")).toString()));
		int iban3 = 98 - iban2;
		if(iban3<10)
		{
			iban = "ES0"+String.valueOf(iban3)+this.Entidad+this.Sucursal+this.Dc+this.NCuenta;
		}
		else
		{
			iban = "ES"+String.valueOf(iban3)+this.Entidad+this.Sucursal+this.Dc+this.NCuenta;
		}
		return iban;
	}
